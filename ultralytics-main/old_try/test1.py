...
def detection_logic_cam1(img, display="Camera1"):
    ...
    if r.obb is not None and len(r.obb.xywhr) > 0:
        ...
        for box, cls_id, conf in zip(boxes, classes, confs):
            ...
            if trigger_line["top_y"] <= cy <= trigger_line["bottom_y"] and object_line["line0"]:
                ...
                lines = [
                    f"{cx_mm, cy_mm, cz_mm / 2, 0, 0, -angle_deg}",
                ]
                # 以写入模式打开文件（会覆盖原文件内容）
            ...
            if cy < trigger_line["top_y"] and object_line["line1"]:
                ...
                if len(dot_list) >= 2:
                    ...
                    print(dot_list)
                    print(cy_max, cy_min, t_max, t_min)
                ...
                if targets["last_line2_flag"] is None:
                    ...
                    send_position_6axis(0, 0, 0, 0, round(SPEED_NOW * 10, 2), -angle_deg, float_order="BADC")
    ...
    if object_line["line2"]:
        ...
        elif 25 <= SPEED_NOW < 40:
            delay_time -= 0.01
        elif 40 <= SPEED_NOW < 45:
            delay_time -= 0.03
        elif 70 <= SPEED_NOW:
            delay_time -= 1
...
def camera_worker(ip, mode, frame_dict, key):
    ...
    try:
        ...
        # 退出前彻底释放资源
        print("[CameraWorker] 停止取流并释放资源...")
        cam.MV_CC_StopGrabbing()
        cam.MV_CC_CloseDevice()
        cam.MV_CC_DestroyHandle()
        print("[CameraWorker] 相机句柄已销毁。")
...
if __name__ == "__main__":
    with open(r"D:\examples_project\python projects\location.txt", "r+", encoding="utf-8") as f:
        f.truncate(0)  # 截断为0字节，清空内容
        print("文件已清空！")
    cam = CameraHandler()
    cam.open_camera()
