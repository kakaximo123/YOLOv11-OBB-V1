...
from chen.Python.get_height import CameraHandler
...
# ========== 新增：cam1 推理队列 ==========
frame_queue_cam1 = queue.Queue(maxsize=3)  # 有界队列，队满时丢帧以保证实时性
...
frame_queue_cam1 = queue.Queue(maxsize=3)
...
def inference_worker():
    global frame_queue_cam1, inference_results, stop_event, model, inference_lock
    while not stop_event.is_set():
        try:
            frame = frame_queue_cam1.get(timeout=0.1)
        except queue.Empty:
            continue

        # ROI 提取
        roi_x_left, roi_x_right = 20, 1660
        roi = frame[:, roi_x_left:roi_x_right].copy()

        with inference_lock:
            try:
                with torch.no_grad():
                    results = model(roi, device="cuda", conf=0.8)
            except Exception as e:
                print("[ERROR] 推理失败:", e)
                results = None
...
def detection_logic_cam1(img, display="Camera1"):
    ...
    r = inference_results["results"][0] if inference_results["results"] is not None else None
    roi = inference_results["roi"]
    h, w = img.shape[:2]
    roi_x_right = 1660
    real_width_cm = 60.09
    real_height_cm = 74.00
    pixels_per_cm_x = (roi_x_right - 20) / real_width_cm
    ...
    # 检测框处理
    if r is not None and r.obb is not None and len(r.obb.xywhr) > 0:
        ...
        for box, cls_id, conf in zip(boxes, classes, confs):
            ...
            rect = ((cx, cy), (w, h), math.degrees(angle))
            ...
            if targets is None:
                targets = {"last_line1_time": None, "last_line2_time": None,
                           "last_line1_flag": None, "last_line2_flag": None,
                           "frame_count": 0, "trajectory": []}
            ...
            # Label 绘制
            cv2.putText(img, f"({cx_mm:.1f},{cy_mm:.1f},{cz_mm:.1f})",
                        (int(cx + 5), int(cy + 40)), cv2.FONT_HERSHEY_TRIPLEX, 1.0, (0, 255, 0), 2)
            ...
            if trigger_line["top_y"] <= cy <= trigger_line["bottom_y"] and object_line["line0"]:
                ...
                send_position_6axis(cx_mm, cy_mm, cz_mm, 0, 0, -angle_deg, float_order="BADC")
                print("发送跟随坐标，坐标信息为(中心x，中心y，高度，角度)", {cx_mm, cy_mm, cz_mm, angle_deg})
            ...
            if cy < trigger_line["top_y"] and object_line["line1"]:
                ...
                if targets["last_line2_flag"] is None:
                    ...
                    for i in range(1, 5):
                        send_position_6axis(cx_mm, cy_mm, cz_mm, 0, SPEED_NOW * 10, -angle_deg, float_order="BADC")
                    for i in range(1, 5):
    ...
    if object_line["line2"]:
        ...
        elif 30 <= SPEED_NOW < 50:
            delay_time -= 0.3
        elif 50 <= SPEED_NOW < 70:
            delay_time -= 0.6
        elif 70 <= SPEED_NOW:
            delay_time -= 1
...
def camera_worker(ip, mode, frame_dict, key):
    ...
    while not stop_event.is_set():
        ...
        try:
            ...
            cam_local = MvCamera()
            cam_local.MV_CC_CreateHandle(sel_info)
            cam_local.MV_CC_OpenDevice()

            # 获取 payload 等信息
            stParam = MVCC_INTVALUE()
            cam_local.MV_CC_GetIntValue("PayloadSize", stParam)
            payload_size = stParam.nCurValue
            data_buf = (ctypes.c_ubyte * payload_size)()
            frame_info = MV_FRAME_OUT_INFO_EX()

            cam_local.MV_CC_StartGrabbing()
            print(f"[{mode}] 已连接相机 IP={ip}")

            last_frame_time = time.time()
            # 拉帧循环
            while not stop_event.is_set():
                ret = cam_local.MV_CC_GetOneFrameTimeout(data_buf, payload_size, frame_info, 1000)
                if ret == 0:
                    ...
                    # --------- 关键改动：cam1 的推理放到单独线程 ---------
                    if mode == "cam1":
                        # 把原始图像放入推理队列（非阻塞，队列满则丢帧）
                        try:
                            img1 = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
                            img_out = detection_logic_cam1(img1)
                            frame_queue_cam1.put_nowait(img1)
                        except queue.Full:
                            # 队列满，丢弃该帧，保证相机不卡住
                            pass

                        # 为保证 UI 有画面可显示：如果还没 processed frame，则直接显示原帧
                        with frame_lock:
                            if frame_dict.get(key) is None:
                                frame_dict[key] = img
                    else:
                        # cam2 继续原有行为（同步检测）
            ...
            try:
                if cam_local is not None:
                    try:
                        cam_local.MV_CC_StopGrabbing()
                    except Exception:
                        pass
                    try:
                        cam_local.MV_CC_CloseDevice()
                    except Exception:
                        pass
                    try:
                        cam_local.MV_CC_DestroyHandle()
                    except Exception:
                        pass
                print(f"[{mode}] 资源已释放")
            except Exception as e:
    ...
    def update_frames(self):
        for key, label in [("cam1", self.label1), ("cam2", self.label2)]:
            ...
            if frame is not None:
                # frame 可能是 np.ndarray BGR or already RGB in detection_logic
                try:
                    rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
                except Exception:
                    # 如果 detection_logic 返回的是已经转为 RGB 的图像，避免重复转换错误
                    rgb = frame if frame is not None else None
                    if rgb is None:
                        continue
                h, w, ch = rgb.shape
...
def main():
    ...
    t_infer_cam1 = threading.Thread(target=inference_worker, daemon=True)
