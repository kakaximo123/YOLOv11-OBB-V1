...
class CoalAttention(nn.Module):
    """
    适用于煤炭背景下杂物检测的轻量级通道+空间注意力模块。

    思路：
    - 通道维度上使用全局平均池化和最大池化，聚焦“高响应”特征通道；
    - 空间维度上对通道聚合后进行 7x7 卷积，强调局部粗糙纹理与不规则形状区域；
    - 兼顾大块煤炭区域与小块杂物之间的对比信息。
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # 通道注意力：强调对煤炭纹理和杂物差异敏感的通道
        avg = self.avg_pool(x)
        mx = self.max_pool(x)
        ch = self.channel_mlp(avg + mx).sigmoid()
        x = x * ch

        # 空间注意力：强调局部粗糙、不规则的小目标区域
        max_out, _ = torch.max(x, dim=1, keepdim=True)
        avg_out = torch.mean(x, dim=1, keepdim=True)
        sa = self.spatial_conv(torch.cat([avg_out, max_out], dim=1)).sigmoid()
        return x * sa
...
class CoalC2f(C2f):
    ...
    基于 C2f 的“煤炭场景增强”模块：
    - 保留 C2f 的高效残差堆叠结构；
    - 在输出端增加 CoalAttention，使特征更关注杂物纹理与边缘。
    """
    ...
    def __init__(
        ...
        self.coal_attn = CoalAttention(c2)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        y = super().forward(x)
        return self.coal_attn(y)
...
class C2fCAA(C2f):
    """
    集成CAA注意力的C2f模块，建议放置在Backbone的末端或Neck部分。
    """

    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        # 在C2f的输出端增加CAA注意力
        self.attn = CoalAnomalyAttention(c2)
