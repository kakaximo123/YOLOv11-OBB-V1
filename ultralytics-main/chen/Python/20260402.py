...
Model = YOLO(r'E:\cumt\jianza\ultralytics-main\ultralytics-main\runs\obb\train26\weights\best.pt')
...
class ImageHandler:
    ...
    def __init__(self, model, modbus=None, conf_thresh=0.6, device="cuda"):
    ...
    def _compute_speed_mm_per_s(self):
        ...
        cy_vals = np.array([p[0] for p in points], dtype=np.float64)
        t_vals = np.array([p[1] for p in points], dtype=np.float64)

        unique_mask = np.concatenate(([True], np.diff(t_vals) > 1e-6))
        cy_vals = cy_vals[unique_mask]
        t_vals = t_vals[unique_mask]

        if len(cy_vals) < self.speed_fit_min_points:
            return 0.0

        if abs(cy_vals[-1] - cy_vals[0]) < self.speed_min_displacement_px:
            return 0.0

        try:
            v_px_per_s, _ = np.polyfit(t_vals, cy_vals, 1)
        ...
        if r.obb is not None and len(r.obb.xywhr) > 0:
            ...
            for box, cls_id, conf in zip(boxes, classes, confs):
                ...
                if depth_img is not None:
                    ...
                    # depth_error = -abs(int(depth_base[abs_cx, abs_cy]) - int(depth_base[1000, abs_cy]))
                    self.dep = depth_length
                    print("真实高度")
                    # print(int(depth_base[abs_cy, abs_cy]), int(depth_img[abs_cy, abs_cx]))
                    # print("0000000000000000000000000000", depth_length, depth_error)
                    self.cz_mm = self.dep /2
                    if abs(self.cz_mm) < 40:
                        self.cz_mm = -40
                    if abs(self.cz_mm) > 500:
                        self.cz_mm = -500
                ...
                if cx > 100 and not self.object_line["line0"]:
                ...
                if cx > 500 and self.object_line["line1"]:
                    ...
                    if self.targets["last_line2_flag"] is None:
                        ...
                        if self.SPEED_NOW < 0 or self.SPEED_NOW > 800:
                            self.SPEED_NOW = 300
                            print("速度超界")
                        if self.modbus is not None and hasattr(self.modbus, "send_all_flag"):
                            try:
                                ...
                                self.modbus.send_position_6axis(cx_mm, cy_mm, self.cz_mm, int(self.SPEED_NOW), 0, -angle_deg + 90)
                            except Exception as e:
                                print("[WARN] send_all_flag failed:", e)
                        ...
                        if self.SPEED_NOW > 300:
                            self.delay_time = self.delay_time + 0.18
