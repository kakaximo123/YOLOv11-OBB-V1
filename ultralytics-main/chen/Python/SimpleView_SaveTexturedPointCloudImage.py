...
from Mv3dRgbdImport.Mv3dRgbdDefine import DeviceType_Ethernet, DeviceType_USB, DeviceType_Ethernet_Vir, DeviceType_USB_Vir, MV3D_RGBD_FLOAT_EXPOSURETIME, \
    ...
    PointCloudType_Common,PointCloudType_Normals ,PointCloudType_Texture, PointCloudType_Texture_Normals, MV3D_RGBD_ENUM_POINT_CLOUD_OUTPUT,PointCloudFileType_PLY, \
    ImageType_PointCloud, ImageType_PointCloudWithNormals, ImageType_TexturedPointCloud, ImageType_TexturedPointCloudWithNormals, ImageType_RGB8_Planar,ImageType_YUV422, PointCloudFileType_PCD_ASCII, PointCloudFileType_PLY
...
if __name__ == "__main__":
    ...
    if MV3D_RGBD_OK == ret:
        ...
        pstPointCloudImage = MV3D_RGBD_IMAGE_DATA()
        pstTexture = MV3D_RGBD_IMAGE_DATA()
        for i in range(0, stFrameData.nImageCount):
            ...
            if ImageType_RGB8_Planar == stFrameData.stImageData[i].enImageType:
                print("Get rgb8 planar succeed: framenum (%d) height(%d) width(%d) len(%d)!" %(stFrameData.stImageData[i].nFrameNum,
                        stFrameData.stImageData[i].nHeight, stFrameData.stImageData[i].nWidth, stFrameData.stImageData[i].nDataLen))
                pstTexture = stFrameData.stImageData[i]
            if ImageType_YUV422 == stFrameData.stImageData[i].enImageType:
                print("Get YUV422 succeed: framenum (%d) height(%d) width(%d) len(%d)!" %(stFrameData.stImageData[i].nFrameNum,
                        stFrameData.stImageData[i].nHeight, stFrameData.stImageData[i].nWidth, stFrameData.stImageData[i].nDataLen))
                pstTexture = stFrameData.stImageData[i]
        # ch:保存纹理点云图像 | en: Save textured point cloud image
        if ImageType_PointCloud == pstPointCloudImage.enImageType and ImageType_RGB8_Planar == pstTexture.enImageType:
            chFileName =  "[{:d}]_TexturedPointCloudImage".format(pstPointCloudImage.nFrameNum)
            ret = camera.MV3D_RGBD_SaveTexturedPointCloudImage(pointer(pstPointCloudImage), pointer(pstTexture), PointCloudFileType_PLY, chFileName)
            if MV3D_RGBD_OK == ret:
