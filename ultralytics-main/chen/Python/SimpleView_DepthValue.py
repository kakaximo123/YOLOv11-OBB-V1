...
# from chen.Python.Mv3dRgbdImport.Mv3dRgbdDefine import *
# from chen.Python.Mv3dRgbdImport.Mv3dRgbdApi import *
    ...
    if MV3D_RGBD_OK == ret:
        ...
        for i in range(0, stFrameData.nImageCount):
            if ImageType_Depth == stFrameData.stImageData[i].enImageType:
                ...
                # ch:解析深度图中点深度 | en:Parse depth image center depth value
                sValue = struct.unpack('H' * int(len(strMode) / 2), strMode)
                # nCenterX = int(stFrameData.stImageData[i].nWidth / 2)💚💚💚💚💚
                # nCenterY = int(stFrameData.stImageData[i].nHeight / 2)💚💚💚💚💚
                nCenterX = int(x)
                nCenterY = int(y)
                print("-------- Depth Image Center: (%d, %d); Depth Value: %d mm" % (
                nCenterX, nCenterY, sValue[nCenterX + nCenterY * stFrameData.stImageData[i].nWidth]))
                real_h = sValue[nCenterX + nCenterY * stFrameData.stImageData[i].nWidth]
                print("real_h", real_h)
